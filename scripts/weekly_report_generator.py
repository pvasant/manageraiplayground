...
# --- Configuration ---
JIRA_PROJECT = os.getenv("JIRA_PROJECT", "YOUR_PROJECT")
JIRA_COMPONENTS = os.getenv("JIRA_COMPONENTS", "component1,component2").split(",")
DAYS_PAST = int(os.getenv("DAYS_PAST", "7"))
...
if not all(required_vars):
    ...
    print("Optional: EMAIL_USER, EMAIL_PASSWORD, MANAGER_EMAIL, SMTP_SERVER, SMTP_PORT")
    exit(1)

# --- Jira API Interaction ---
def fetch_jira_issues():
    """
    Connects to Jira using a bearer token and fetches issues from the configured time period.
    """
    try:
        ...
        components_str = ", ".join([f'"{c.strip()}"' for c in JIRA_COMPONENTS])
        jql_query = (
...
def analyze_status_transitions(issues):
    """
    Analyze which issues moved to specific statuses in the configured time period
    """
    status_transitions = {
    ...
    for issue in issues:
        ...
        for history in histories:
            ...
            # Only consider changes in the configured time period
...
def generate_report_with_gemini(status_transitions):
    ...
    prompt_text = f"""
You are a technical project manager creating a weekly status report for the project team. 
...
Report covers {JIRA_PROJECT} project activity for {', '.join(JIRA_COMPONENTS)} components from {(datetime.now() - timedelta(days=DAYS_PAST)).strftime('%B %d, %Y')} to {datetime.now().strftime('%B %d, %Y')}.
"""
...
def send_report_email(report_content, jira_issues_count):
    ...
    try:
        ...
        msg['Subject'] = f"Weekly Status Report - {JIRA_PROJECT} Team ({datetime.now().strftime('%B %d, %Y')})"
        
        # Email body
        email_body = f"""
Dear Team,

Please find the weekly status report for the {JIRA_PROJECT} project team below.
...
"""
        ...
        smtp_configs = [
            # Company internal SMTP (no auth)
            {"server": SMTP_SERVER, "port": 25, "use_auth": False, "use_tls": False},
            # Company internal SMTP with TLS
            {"server": SMTP_SERVER, "port": SMTP_PORT, "use_auth": False, "use_tls": True},
            # Company SMTP with authentication
            {"server": SMTP_SERVER, "port": SMTP_PORT, "use_auth": True, "use_tls": True},
            # Gmail fallback
...
def main():
    """
    Main function to generate and send weekly status report
    """
